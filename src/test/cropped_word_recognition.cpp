...
int main(int argc, char* argv[])
{
...
#if 0
    string vocabulary = "0123456789"; // must have the same order as the classifier output classes
    lexicon.push_back(string("19"));
#else
    string vocabulary = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"; // must have the same order as the classifier output classes
    lexicon.push_back(string("Tillhor"));
    lexicon.push_back(string("beslut"));
    lexicon.push_back(string("TJM"));
    lexicon.push_back(string("BYGGNADSNAMNDEN"));
    lexicon.push_back(string("Molndals"));
    lexicon.push_back(string("kommun"));
#endif
    ...
    Ptr<OCRBeamSearchDecoder> ocr = OCRBeamSearchDecoder::create(
								 loadOCRBeamSearchClassifierCNN("../data/OCRBeamSearch_CNN_model_data.xml.gz"),
